...
<div class="col-md-12"> 
    ...
    <div class="panel panel-primary" style="border-color: green;">
        @if (isset($orderStart))
            <div class="panel-heading" style="background-color: gray;">Thống kê đơn hàng từ @php echo $orderStart; @endphp đến ngày @php echo $orderFinish; @endphp</div>
        @else
            <div class="panel-heading" style="background-color: gray;">Thống kê đơn hàng</div>
        @endif
        

        <div class="panel-body">
            @if (isset($totalRevenue))
                @foreach ($totalRevenue as $rows)
                    <div style="color: black; margin:10px; font-size:16px; font-weight:bold"> Tổng doanh thu: {{ number_format($rows->total) }}đ</div>
                @endforeach 
            @endif
            ...
            <table class="table table-bordered table-hover" style="color: black; border: 2px solid green; margin-top: 10px; ">
                ...
                @if (isset($orderStatistics))
                    @foreach($orderStatistics as $rows)
                    <tr>
                        <td>{{ $rows->fullname }}</td>
